...
const PinDetail = ({user}) => {
  ...
  const fetchPinDetail=()=>{
    ...
    if(query){
      // getting other pins with similar category as this pin
      client.fetch(query)
      .then((data)=>{
        setpinDetail(data[0]);
        if(data[0]){
      ...
      <div className='w-full p-5 flex-1 xl:min-w-620'>
         <div className='items center justify-between' >
          <div className='flex gap-2 items-center'> 
            <a
            href={`${pinDetail.image?.asset?.url}?dl=`}// allows to download that specific image
            download
         ...
         <div>
           <h1 className='text-4xl font-bold mt-3 break-words'>  {pinDetail.title}</h1>
